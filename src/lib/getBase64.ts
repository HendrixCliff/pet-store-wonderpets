...
async function getBase64(imageUrl: string) {
        ...
        try {
            ...
            if (!res.ok) {
                throw new Error(`Failed to fetch image: ${res.statusText} ${res.statusText}`)
            }
...
export default async function addBlurreddataUrls(images: 
    ImagesResults): Promise<Photo[]> {
        // Make all request at once instead of  awaiting each one -avoiding a
        //waterfall
        const base64Promises = images
        .photos.map(photo => getBase64(photo.src.large))
